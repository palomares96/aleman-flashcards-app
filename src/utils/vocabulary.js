import { suggestedGrammar } from "../data/grammarCatalog.js";
import {
  LEARNING_VERSION,
  USAGE_NOTES,
  VERB_FAMILIES,
...
export function normalizeGerman(value, type) {
  ...
  if (type === "noun")
    return term.charAt(0).toLocaleUpperCase("de-DE") + term.slice(1);
  if (type === "verb") {
    const match = term.match(/^(sich\s+)?([\p{L}|·-]+)$/iu);
    if (match)
      return (
        (match[1] ? "sich " : "") +
        match[2].replace(/[|·-]/g, "").toLocaleLowerCase("de-DE")
      );
  }
  if (type === "adjective" && /^[\p{L}-]+$/u.test(term))
    ...
    return [
      {
        ...
        prefix,
        meaning,
        ...(own(item, "exampleDe") ? { exampleDe: clean(item.exampleDe) } : {}),
        ...(own(item, "exampleEs") ? { exampleEs: clean(item.exampleEs) } : {}),
      },
    ];
  });
}
...
export function getSuggestedLearning(word) {
  ...
  return (
    USAGE_NOTES[`${word.type}:${german}`] ||
    (word.type === "verb" && german.startsWith("sich ")
      ? USAGE_NOTES[`verb:${german.slice(5)}`]
      : undefined)
  );
}
...
export function normalizeLearning(learning = {}) {
  ...
  return {
    ...
    alternativesDe: Array.isArray(learning.alternativesDe)
      ? [
          ...new Set(
            learning.alternativesDe
      ...
      : [],
    alternativesEs: Array.isArray(learning.alternativesEs)
      ? [
          ...new Set(
            learning.alternativesEs
              .filter((value) => typeof value === "string")
              .map(clean)
              .filter(Boolean),
          ),
...
export function enrichWord(word) {
  ...
  if (word.type === "verb") {
    let prefixes = own(attributes, "separablePrefixes")
      ? attributes.separablePrefixes
      : VERB_FAMILIES[german];
    if (prefixes) {
      ...
      if (prefixes.length && !attributes.prefixCatalogVersion) {
        ...
        prefixes = [
          ...prefixes,
          ...seeds.filter((item) => !seen.has(item.prefix)),
        ];
      }
      attributes.separablePrefixes = prefixes;
      attributes.prefixCatalogVersion = Math.max(
        LEARNING_VERSION,
        Number(attributes.prefixCatalogVersion) || 0,
      );
    }
  }
  const result = { ...word, german, attributes };
  if (!own(word, "grammar") && suggestedGrammar(result))
    result.grammar = {
      ...suggestedGrammar(result),
      version: 1,
      source: "grammar-catalog-v1",
    };
...
export function planVocabularyPatch(word) {
  ...
  if (
    word.type === "verb" &&
    newPrefixes &&
    JSON.stringify(oldPrefixes) !== JSON.stringify(newPrefixes)
  ) {
    patch["attributes.separablePrefixes"] = newPrefixes;
  }
  if (
    word.type === "verb" &&
    enriched.attributes.prefixCatalogVersion &&
    enriched.attributes.prefixCatalogVersion !==
      word.attributes?.prefixCatalogVersion
  ) {
    patch["attributes.prefixCatalogVersion"] =
      enriched.attributes.prefixCatalogVersion;
  }
  if (!own(word, "learning") && enriched.learning)
    patch.learning = enriched.learning;
  if (!own(word, "grammar") && enriched.grammar)
    patch.grammar = enriched.grammar;
  if (Object.keys(patch).length)
    patch.schemaVersion = Math.max(3, Number(word.schemaVersion) || 0);
  return patch;
}
...
export function applyVocabularyPatch(word, patch) {
  ...
  for (const [field, value] of Object.entries(patch)) {
    if (field.startsWith("attributes."))
      result.attributes[field.slice(11)] = value;
...
export function expandVocabulary(words) {
  return words.flatMap((original) => {
    ...
    const derived = normalizePrefixes(
      word.attributes.separablePrefixes,
    ).flatMap((prefix) => {
      const german = composeSeparableVerb(word.german, prefix.prefix);
      if (!german) return [];
      const card = {
        ...word,
        id: `${word.id}_${prefix.prefix}`,
        baseWordId: word.id,
        german,
        spanish: prefix.meaning,
        isDerived: true,
      ...
      delete card.grammar;
      if (suggestedGrammar(card))
        card.grammar = {
          ...suggestedGrammar(card),
          version: 1,
          source: "grammar-catalog-v1",
        };
      const usage = prefix.learning || getSuggestedLearning(card);
      if (usage || prefix.exampleDe)
        card.learning = normalizeLearning({
          ...usage,
...
export function prepareWordForSave(word) {
  ...
  result.schemaVersion = Math.max(3, Number(word.schemaVersion) || 0);
  if (!result.german || !result.spanish)
    throw new Error("Escribe la palabra en alemán y su traducción.");
