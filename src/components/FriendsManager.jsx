...
// --- NOVEDAD: Añade un icono para la importación ---
const ImportIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>;
...
function FriendsManager({ user, userProfile }) {
    ...
    // --- NOVEDAD: Estado para gestionar el modal de importación ---
    const [showImportModal, setShowImportModal] = useState(false);
    const [selectedFriend, setSelectedFriend] = useState(null);

    // ... (el resto de tus useEffect, handleSearch, sendFriendRequest, handleRequest, etc. no cambian) ...
    useEffect(() => {
        if (!user) return;
        setLoading(true);
...
    };
    ...
    // --- NOVEDAD: Funciones para abrir y cerrar el modal ---
    const handleOpenImportModal = (friend) => {
    ...
    const TabButton = ({ tabName, label }) => ( <button onClick={() => setActiveTab(tabName)} className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors ${activeTab === tabName ? 'bg-gray-800 text-white' : 'bg-gray-900 text-gray-400 hover:bg-gray-800/50'}`} > {label} </button> );
    
    return (
        <div className="w-full max-w-4xl mx-auto">
            {/* --- NOVEDAD: Renderiza el modal si está activo --- */}
            {showImportModal && selectedFriend && (
                <ImportModal 
                    user={user} 
            ...
            <div className="p-6 bg-gray-800 rounded-b-lg min-h-[300px]">
                    ...
                    <div className="space-y-3">
                        {loading ? <p>Cargando...</p> : friends.length > 0 ? friends.map(friend => (
                            <div key={friend.id} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                                ...
                                {/* --- NOVEDAD: Botón para iniciar la importación --- */}
                                <button 
                                    onClick={() => handleOpenImportModal(friend)}
                                    className="flex items-center gap-2 px-3 py-1 text-sm text-white bg-indigo-600 hover:bg-indigo-500 rounded-md transition-colors"
                                    title={`Importar palabras de ${friend.displayName}`}
                                >
                                    <ImportIcon />
                                    Importar
                                </button>
                ...
                {/* ... (El resto de tu JSX para las pestañas de 'requests' y 'search' no cambia) ... */}
                 {activeTab === 'requests' && (
                     <div className="space-y-3">
